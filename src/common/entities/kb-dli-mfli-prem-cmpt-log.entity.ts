...
@Entity({
  name: 'tb_kb_dli_mfli_prem_cmpt_logs',
  comment: 'KB손보 API 재난배상&다중이용업소 API(보험료산출) 통신 로그 테이블',
})
@Index(['insdpsIdno'])
export class KbDliMfliPremCmptLog {
  @PrimaryGeneratedColumn({
    name: 'seq_no',
    comment: 'ID',
    type: 'int',
  ...
  @Column({
    name: 'rate_calc_fndtn_cnt_res',
    comment: '요율산출기초수 응답',
    type: 'varchar',
    length: 28,
    nullable: true,
  })
  rateCalcFndtnCntRes: string;
