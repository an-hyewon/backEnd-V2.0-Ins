...
@Entity({
  name: 'tb_kb_dli_mfli_grup_ctr_cclu_logs',
  comment:
    'KB손보 API 재난배상&다중이용업소 API(단체계약체결) 통신 로그 테이블',
})
@Index(['apcno'])
@Index(['pcno'])
@Index(['purps'])
export class KbDliMfliGrupCtrCcluLog {
  @PrimaryGeneratedColumn({
    name: 'seq_no',
    comment: 'ID',
    type: 'int',
  ...
  @Column({
    name: 'srdng_bldg_gap_dist',
    comment: '주위건물이격거리(방화담/경계기준)',
    type: 'varchar',
    length: 2,
    nullable: true,
  })
  srdngBldgGapDist: string;

  @Column({
    name: 'pymn_stmd_cd',
    comment: '납입결제방법코드(11 : 가상계좌 15 : 신용카드)',
    type: 'varchar',
    length: 2,
    nullable: true,
  })
